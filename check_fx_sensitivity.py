...
def find_fx_sensitivity_excerpt(full_text: str) -> tuple:
    ...
    #
    # 주의: "민감도"라는 단어는 보통 표(구체적 수치) "앞"의 제목("...환율 민감도
    # 분석은 다음과 같습니다")에 나오기도 하지만, 회사에 따라 표가 먼저 나오고
    # 그 표를 설명하는 문장("상기 민감도 분석은 ...를 대상으로 하였습니다")이
    # "뒤"에 나오는 경우도 많다(예: 삼성전기). 이런 경우 "민감도" 앞쪽으로
    # 충분히 넓게 잡지 않으면, 정작 중요한 표의 숫자(USD/EUR 등 통화별 금액)를
    # 놓치고 표 뒤의 안내 문장만 발췌해버려서 "구체적 수치 없음"으로 잘못
    # 판단하게 된다. 그래서 앞쪽 범위를 넉넉하게 잡는다(뒤쪽에 표가 있는 경우도
    # 있으므로 뒤쪽도 어느 정도 넓게 잡는다). 발췌문이 좀 길어져도 Claude에게는
    # 이 부분만 보내므로 비용 부담은 크지 않다.
    for m in re.finditer("민감도", full_text):
        start = max(0, m.start() - 3000)
        end = min(len(full_text), m.start() + 2000)
...
간단히 요약하세요. 특히 통화별(USD, EUR, JPY 등)로 "X% 상승/하락 시 세전손익
얼마 증가/감소" 같은 숫자가 담긴 표가 있는지 발췌문 전체를 꼼꼼히 확인하세요 -
이런 표는 발췌문의 앞쪽이 아니라 뒤쪽에 나올 수도 있습니다. 표가 있다면
"당(반)기말"(가장 최근 시점) 기준, 가장 비중이 큰 통화(보통 USD)와 "계"(합계)
행의 수치, 그리고 몇 %를 기준으로 한 것인지를 반드시 포함해서 답하세요
(예: "환율 10% 상승 시 세전손익 약 30억원 감소, 10% 하락 시 약 30억원 증가").
표를 아무리 찾아봐도 구체적인 수치 없이 서술만 있다면 "구체적 수치 없이 위험관리
