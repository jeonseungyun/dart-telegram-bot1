...
증권사들이 거의 매일 형식적으로 내는 ELB/ELS 등 파생결합증권 발행 공시처럼, 투자 판단에 별 의미가 없는 반복성 공시는 기본값으로 이미 알림에서 제외되어 있습니다. 별도 설정 없이도 아래 키워드가 제목에 포함된 공시는 자동으로 걸러집니다.

```
증권발행실적보고서, 일괄신고추가서류, 일괄신고서, 효력발생안내, 파생결합증권, 파생결합사채, ELB, ELS, DLS, DLB
```
